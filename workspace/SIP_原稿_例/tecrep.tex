...
\item
\verb/\tecrepyear/ に研究報告の発表年を指定します．
...
\item
和文の「あらまし（300文字程度）」「キーワード」は，\texttt{jabstract} 環境，
...
「和文論文誌への投稿方法について」の「投稿のしおり」
（\url{https://www.ieice.org/jpn_r/submission/ronbunshi.html}），
またはバックナンバーを参照してください．
...
\texttt{citesort.sty} に手を加えたものを使用しています．
...
\verb/\RN/，\verb/\FRAC/，\verb/\kintou/，
\verb/\ruby/%，\verb/\MARU/ 
というマクロ\cite{tex,Okumura8}を
...
A = \left(
 ...
 \label{eq:ex2}
\end{equation}
式 (\ref{eq:ex1}) と式 (\ref{eq:ex2}) を比べてみてください．
...
%
%\noindent
%{\bfseries 例5}\hskip1zw \verb/\quad/ の定義を変える．
%
...
%%%（以下では段幅の関係で折り返します）．
%\begin{verbatim}
%dvips -Pprinter -t a4 -O 0in,0in -o file.ps file.dvi
%\end{verbatim}
%\texttt{printer} には，使用するプリンタ名を記述します．
%オプションの \texttt{-t a4 -O 0in,0in} は省略できます．
%
%次に Acrobat Distiller（有料アプリ）で PDF に変換します．
%\end{enumerate}
%
%\item
